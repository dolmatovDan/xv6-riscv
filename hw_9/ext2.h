...
static inline Ext2
ext2_open(const char *path)
{
    Ext2 e;
    e.fd = open(path, O_RDONLY);
    if (e.fd < 0) { perror(path); exit(1); }
    if (pread(e.fd, &e.sb, sizeof(e.sb), EXT2_SB_OFFSET) != (ssize_t)sizeof(e.sb)) {
...
static inline void
read_block(Ext2 *e, uint32_t bnum, void *buf)
{
    if (bnum == 0) {
        memset(buf, 0, e->block_size);
        return;
    }
    if (pread(e->fd, buf, e->block_size, (off_t)bnum * e->block_size) != (ssize_t)e->block_size) {
        fprintf(stderr, "cannot read block %u\n", bnum); exit(1);
    }
}

static inline struct ext2_inode
read_inode(Ext2 *e, uint32_t inum)
{
    ...
    off_t ino_off = (off_t)le32(gd.bg_inode_table) * e->block_size + (off_t)index * e->inode_size;
