...
static void fmt_time(uint32_t t) {
  time_t ts = (time_t)t;
  char buf[32];
...
static void fmt_mode(uint16_t mode) {
  char t;
  if (S_ISREG(mode))
    t = '-';
    ...
    t = '?';
  printf("%c%c%c%c%c%c%c%c%c%c", t, (mode & S_IRUSR) ? 'r' : '-',
...
static void show_ind_ptrs(Ext2 *e, uint32_t bnum, uint8_t *buf) {
  if (bnum == 0)
    return;
...
int main(int argc, char *argv[]) {
  ...
  printf("atime:       ");
  fmt_time(le32(ino.i_atime));
  printf("\n");
  printf("mtime:       ");
  fmt_time(le32(ino.i_mtime));
  printf("\n");
  printf("ctime:       ");
  fmt_time(le32(ino.i_ctime));
  printf("\n");
