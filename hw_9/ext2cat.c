...
static void
emit(Writer *w, uint32_t bnum)
{
    if (w->remaining == 0) return;
    size_t n = (w->remaining < w->e->block_size)
               ? (size_t)w->remaining
               : (size_t)w->e->block_size;
    if (bnum == 0) {
        memset(w->dbuf, 0, n);
    } else {
        if (pread(w->e->fd, w->dbuf, w->e->block_size, (off_t)bnum * w->e->block_size) < 0) {
            perror("pread"); exit(1);
        }
    }
    write_all(w->dbuf, n);
...
static void
emit_indirect(Writer *w, uint32_t bnum, uint32_t *ibuf)
{
...
static void
emit_dindirect(Writer *w, uint32_t bnum, uint32_t *ib1, uint32_t *ib2)
{
    uint32_t P = w->e->block_size / 4;
    if (bnum == 0) {
        for (uint32_t i = 0; i < P && w->remaining > 0; i++)
...
static void
emit_tindirect(Writer *w, uint32_t bnum, uint32_t *ib1, uint32_t *ib2, uint32_t *ib3)
{
    uint32_t P = w->e->block_size / 4;
    if (bnum == 0) {
        for (uint32_t i = 0; i < P && w->remaining > 0; i++)
...
main(int argc, char *argv[])
{
    ...
    if (w.remaining > 0) emit_indirect(&w,  le32(ino.i_block[12]), ib1);
    if (w.remaining > 0) emit_dindirect(&w, le32(ino.i_block[13]), ib1, ib2);
    if (w.remaining > 0) emit_tindirect(&w, le32(ino.i_block[14]), ib1, ib2, ib3);

    free(w.dbuf);
    free(ib1); free(ib2); free(ib3);
    close(e.fd);
    return 0;
}
